    ...
    try:
        ...
        if model.status == GRB.OPTIMAL:
            ...
            with open("pap_pli_resultados.csv", 'a', newline = '') as file:
                ...
                writer.writerow([model.ModelName, model.ObjVal, model.Runtime, model.IterCount, model.NumVars, model.NumConstrs, model.NumObj])
        ...
        if path.exists(dir):
            ...
            for entry in obj :
                if entry.is_dir() or entry.is_file():
                    ...
                    if len(f_dict) == 0:
                        sys.exit("Erro: O diretorio {} nao possui arquivos .pap\n".format(dir))
            #print(f_dict.items())

            # Cria arquivo .csv para armazenar os resultados
            with open("pap_pli_resultados.csv", 'a', newline = '') as file:
                writer = csv.writer(file)
                writer.writerow(["Instancia", "Valor Obj", "Tempo", "# Iteracoes", "# Variaveis", " # Restricoes", "# Obj"])
            ...
            for inst in f_dict:
                ...
                solve(param[0], param[1], param[2], param[3], param[4], param[5], param[6], param[7], inst[:-4])
        else:
